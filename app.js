...
const form = document.querySelector('.paras');
const number = document.getElementById('number');
const result = document.querySelector('.result');

form.addEventListener('submit',function(e){
  e.preventDefault();
 const value = parseInt(number.value); //parseint is used to change the data type of value to number
 const random = Math.floor(Math.random()*text.length);
if (isNaN(value)|| value <= 0 || value > 9){ // here this tells if we need less than 0 paras or 1 para or more than 9 paras it gives onl one para as result 
